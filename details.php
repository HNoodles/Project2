...
if (isset($_GET['userID']) && isset($_GET['artworkID'])) {// add to cart attempt
    ...
    // find if has add to cart and redirect to this page by history
    $sqlCart = "SELECT * FROM carts WHERE userID = ".$userID." AND artworkID = ".$artworkID;
    $resultCart = $connection->query($sqlCart);

    if (!$resultCart->fetch_array()) {// not in current user's cart
    ...
    } else {// has in current users cart
